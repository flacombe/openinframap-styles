...
CREATE MATERIALIZED VIEW pdm_project_poteaux AS 
  SELECT osm_primitive_id(osm_id, geometry) AS osm_id, tags->'name' AS name, hstore_to_json(tags) AS tags, geometry AS geom
...
CREATE MATERIALIZED VIEW pdm_project_substations AS 
  SELECT osm_id, name, hstore_to_json(tags) AS tags, geometry AS geom
  FROM (
    SELECT osm_primitive_id(osm_id, geometry) AS osm_id, tags->'name' AS name, tags, geometry
    FROM osm_power_substation
    where tags->'operator'='Enedis' AND tags->'substation'='minor_distribution'
    UNION SELECT osm_primitive_id(osm_id, geometry) AS osm_id, tags->'name' AS name, tags, geometry
